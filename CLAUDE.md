...
Learned from diffing the `login-page` and `signup-page` branches (see `WORKFLOW.md`): `signup-page` shipped a `RegistrationPage` missing the design's `teamName` field, and it passed build + lint anyway.

- A new auth/marketing page must include every field present in its source `UI-UX Design/<Page>/index.html`. Before requesting review, diff the component's input `id`s against that file's — a green `npm run build`/`lint` does not catch a silently dropped field.
- The design's behavior lives in its `scripts.js`, not just its `index.html`/`styles.css`. Port and review the script alongside the markup — validation rules (e.g. password requirements), live-region strategy, and `aria-describedby` composition are defined there.
- Ship a new page as an additive-only PR: a new `src/pages/<Page>/` folder plus a minimal `App.tsx` route change (see `signup-page`'s 3-file, zero-deletion diff). Land shared infra — tokens, theme provider, routing setup — in its own PR first rather than bundling it with the first feature that needs it (see `login-page`'s 21-file, mixed infra+feature commit, which is materially harder to review).
